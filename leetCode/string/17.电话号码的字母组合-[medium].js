...
var letterCombinations = function(digits) {
  ...
  console.log('log => : letterCombinations -> strs', strs)

  const combination = arr => {
    const strs = []
    for (const letterA of (arr[0] ? arr[0] : [])) {
      for (const letterB of (arr[1] ? arr[1] : [''])) {
        strs.push(`${letterA}${letterB}`)
      }
    }
    arr.splice(0, 2, strs)
    if (arr.length === 1) {
